...
test("ships social metadata and accessible controls", async () => {
  ...
  assert.match(gameSource, /PRESS SPACE: NEXT LEVEL/i);
  assert.match(gameSource, /PRESS R: REPLAY LEVEL/i);
  assert.match(gameSource, /className="touch-start-copy">NEXT LEVEL/i);
  assert.match(gameSource, /className="touch-start-copy">REPLAY LEVEL/i);
